...
void limites(double t){
	for (int i = 0 ; i < NONDES ; i++){
		bords(ondes[i]);
		limites_mur_rect(ondes[i], 0,  YMAXS/2 - 2, XMAX-1, YMAXS/2 + 2);		
	}

	limites_onde_point(ondes[0], XMAXS/2, YMAXS/4, t); 
	limites_onde_point(ondes[1], XMAXS/2, 3*YMAXS/4, t); 
